...
import { DATA_FEED_CONTRACT_NAME, DEPOSIT_VAULT_CONTRACT_NAME, MIDAS_AC_CONTRACT_NAME, MIDAS_AC_DEPLOY_TAG, MOCK_AGGREGATOR_NETWORK_TAG, ST_USD_CONTRACT_NAME } from '../../config';
...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  if (hre.network.tags[MOCK_AGGREGATOR_NETWORK_TAG]) {
    ...
    const aggregatorDeploy = await (await hre.ethers.getContractFactory('AggregatorV3Mock', owner)).deploy()
