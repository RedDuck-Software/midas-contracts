...
describe('stUSD', function () {
  ...
  describe('pause()', () => {
    it('should fail: call from address without ST_USD_PAUSE_OPERATOR_ROLE role', async () => {
      ...
      await expect(stUSD.connect(caller).pause())
    ...
    it('should fail: call when already paused', async () => {
      ...
      await expect(stUSD.connect(owner).pause())
    ...
    it('call when unpaused', async () => {
      ...
      expect(await stUSD.paused()).eq(false);
      await expect(stUSD.connect(owner).pause())
        .to.emit(stUSD, stUSD.interface.events['Paused(address)'].name)
        .to.not.reverted
  ...
  describe('unpause()', () => {
    it('should fail: call from address without ST_USD_PAUSE_OPERATOR_ROLE role', async () => {
      const { owner, stUSD, regularAccounts } = await loadFixture(defaultDeploy)
      const caller = regularAccounts[0];

      await stUSD.connect(owner).pause();
      await expect(stUSD.connect(caller).unpause())
        .revertedWith(acErrors.WMAC_HASNT_ROLE);
    })
    ...
    it('should fail: call when already paused', async () => {
      ...
      await expect(stUSD.connect(owner).unpause())
    ...
    it('call when paused', async () => {
      ...
      await expect(stUSD.connect(owner).unpause())
        .to.emit(stUSD, stUSD.interface.events['Unpaused(address)'].name)
        .to.not.reverted
  ...
  describe('mint()', () => {
    it('should fail: call from address without ST_USD_MINT_OPERATOR_ROLE role', async () => {
      ...
      await mint({ stUSD, owner }, owner, 0, { from: caller, revertMessage: acErrors.WMAC_HASNT_ROLE });
    })
  ...
  describe('burn()', () => {
    it('should fail: call from address without ST_USD_BURN_OPERATOR_ROLE role', async () => {
      ...
      await burn({ stUSD, owner }, owner, 0, { from: caller, revertMessage: acErrors.WMAC_HASNT_ROLE });
    })
    ...
    it('should fail: call when user has insufficient balance', async () => {
      ...
      await burn({ stUSD, owner }, to, amount, { revertMessage: 'ERC20: burn amount exceeds balance' });
    })
  ...
  describe('setMetadata()', () => {
    it('should fail: call from address without DEFAULT_ADMIN_ROLE role', async () => {
      const { owner, stUSD, regularAccounts } = await loadFixture(defaultDeploy)
      const caller = regularAccounts[0];
      await setMetadataTest({ stUSD, owner }, 'url', 'some value', { from: caller, revertMessage: acErrors.WMAC_HASNT_ROLE });
    })
  ...
  describe('_beforeTokenTransfer()', () => {
    it('should fail: mint(...) when address is blacklisted', async () => {
      ...
      await mint({ stUSD, owner }, blacklisted, 1, { revertMessage: acErrors.WMAC_HAS_ROLE });
    })
    ...
    it('should fail: burn(...) when address is blacklisted', async () => {
      ...
      await blackList({ blacklistable: stUSD, accessControl, owner }, blacklisted);
      await burn({ stUSD, owner }, blacklisted, 1, { revertMessage: acErrors.WMAC_HAS_ROLE });
    })
    ...
    it('should fail: transfer(...) when to address is blacklisted', async () => {
      ...
      await expect(stUSD.connect(from).transfer(blacklisted.address, 1))
        .revertedWith(acErrors.WMAC_HAS_ROLE);
    })
    ...
    it('should fail: transferFrom(...) when from address is blacklisted', async () => {
      ...
      await expect(stUSD.connect(to).transferFrom(blacklisted.address, to.address, 1))
        .revertedWith(acErrors.WMAC_HAS_ROLE);
    })
    ...
    it('should fail: transferFrom(...) when to address is blacklisted', async () => {
      ...
      await expect(stUSD.connect(caller).transferFrom(from.address, blacklisted.address, 1))
        .revertedWith(acErrors.WMAC_HAS_ROLE);
    })
