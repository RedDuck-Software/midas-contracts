...
describe('WithMidasAccessControl', function () {
  ...
  describe('grantRole()', () => { 
    it('should fail when call from non role admin', async () => {
      ...
      await expect(wAccessControlTester.grantRoleTester(roles.blacklisted, regularAccounts[1].address))
        .reverted;
    })
  ...
  describe('revokeRole()', () => { 
    ...
    it('call from role admin', async () => {
      ...
      expect(await accessControl.hasRole(roles.blacklisted, regularAccounts[1].address)).eq(false)
    })
  })
});