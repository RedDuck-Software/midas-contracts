...
contract mBASIS is ERC20PausableUpgradeable, Blacklistable, IMTbill {
    ...
    function mint(
        address to,
        uint256 amount
    ) external onlyRole(M_BASIS_MINT_OPERATOR_ROLE, msg.sender) {
        _mint(to, amount);
    }
    ...
    function burn(
        address from,
        uint256 amount
    ) external onlyRole(M_BASIS_BURN_OPERATOR_ROLE, msg.sender) {
        _burn(from, amount);
    }
    ...
    function pause()
        external
        override
        onlyRole(M_BASIS_PAUSE_OPERATOR_ROLE, msg.sender)
    {
        _pause();
    }
